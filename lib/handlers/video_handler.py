...
class VideoHandler(tornado.web.RequestHandler):
    ...
    @classmethod
    def set_ws_send(cls, ws_send_func):
        cls.ws_send = ws_send_func
    ...
    def handle_message(cls, message):
        if cls.current_task == None:
            return
        task_id = cls.current_task.get("task_id")
        cls.update_task(message)
        ws_message = message.copy()
        ws_message.update({"task_id":task_id})
        cls.ws_send(ws_message)
